  ...
  var Useraccounts = [
    {
      ...
      phone: "0938412412",
      address:
        "159 Bùi Đình Túy, Phường 24, Quận Bình Thạnh, Thành phố Hồ Chí Minh",
...
function check_login(event) {
  ...
  if (
    isNaN(Number(phone)) ||
    Number(phone) < 100000000 ||
    Number(phone) > 999999999
  ) {
...
function check_register(event) {
  ...
  if (!isValidPassword(password)) {
    register_error(
      "Mật khẩu phải từ 8 kí tự trở lên, có ít nhất một chữ hoa, chữ thường, chữ số."
    );
    return false;
  }
...
function login() {
  if (
    localStorage.getItem("check_admin") !== null &&
